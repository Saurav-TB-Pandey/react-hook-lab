...
8. Generate up to 4 relevant tags for the article. The tags MUST ALWAYS include "reacthooklab".
9. IMPORTANT: Tags must contain ONLY lowercase alphanumeric characters (no hyphens, spaces, underscores, or special characters).
10. Generate a "linkedin_post". This should be a highly engaging, detailed plain-text social media post (with emojis and hashtags, but NO markdown) for a LinkedIn audience. It should be substantial enough to take about 1 minute to read (roughly 150-250 words), pulling readers in with a strong hook, explaining the "why" behind the changes, and ending with a call to action or question. MUST BE EXTREMELY WELL-FORMATTED using blank lines and line breaks to separate ideas and make it scannable.
